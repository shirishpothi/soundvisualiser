...
function performEnhancedThemeTransition() {
  const elementsToAnimate = [
    ...
    {
      selector: '.feature-card, .demo-item, .step-content, .faq-item',
      delay: 100
    },
...
function setupScrollAnimations() {
    ...
    entries => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          if (
            !element.classList.contains('blur-to-focus') &&
  ...
  const containerElements = document.querySelectorAll(
    '.features-grid, .demo-grid, .steps-container, .about-content, .faq-container'
  );
...
function setupButtonEffects() {
  const buttons = document.querySelectorAll(
    '.launch-btn, .primary-button, .secondary-button'
  );
